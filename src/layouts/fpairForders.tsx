...
import { FBotPair, FPairFOrder, TabPanelProps } from "../types";
...
function CustomTabPanel(props: TabPanelProps) {
  ...
  return (
    <div
      ...
      id={`fpair_${parentId}_orders-tabpanel_${index}`}
      aria-labelledby={`fpair_${parentId}_orders-tabpanel_${index}`}
      {...other}
    >
...
function a11yProps(index: number, pairId: number) {
  return {
    id: `fpair_${pairId}-orders_tab-${index}`,
    "aria-controls": `orders-tabpanel-${index}`,
  };
}

const FpairForders: React.FC<FpairFordersProps> = (autoUpdate) => {
  const pair = useRecordContext<FBotPair>(),
  ...
  } = useGetManyReference<FPairFOrder>(
    "forders",
    {
      target: "pair_id",
      id: pair.id,
  ...
  return (
    <Box className="fpairForders pairOrders" sx={{ width: "100%" }}>
      <Box sx={{ borderBottom: 1, borderColor: "divider" }}>
        <Tabs
          value={tabValue}
          onChange={handleTabsChange}
          aria-label="Fpair orders tabs"
          variant="fullWidth"
        >
